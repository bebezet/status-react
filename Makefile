.PHONY: nix-add-gcroots clean nix-clean start-metro react-native-desktop test release _list _fix-node-perms _tmpdir-mk _tmpdir-rm
...
start-clojure: export TARGET := default
start-clojure: ##@start Watch for and build Clojure changes for mobile
	yarn shadow-cljs watch mobile

start-clojure-desktop: export TARGET ?= $(HOST_OS)
start-clojure-desktop: #@start Watch for and build Clojure changes for desktop
	clj -R:dev build.clj watch --platform desktop

start-metro: export TARGET := default
start-metro: ##@start Start Metro to build React Native changes
	@scripts/start-react-native.sh

start-re-frisk: export TARGET := clojure
start-re-frisk: ##@start Start re-frisk server
